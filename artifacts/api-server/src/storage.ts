...
export class DatabaseStorage implements IStorage {
  ...
  async getProperties(filters?: any): Promise<(Property & { agent: any })[]> {
    ...
    return results.map(r => ({ ...r.property, agent: r.agent ? safeUser(r.agent as any) : null }));
  }
  ...
  async getProperty(id: number): Promise<(Property & { agent: any }) | undefined> {
    ...
    return { ...results[0].property, agent: results[0].agent ? safeUser(results[0].agent as any) : null };
  }
  ...
  async createSavedSearch(userId: string, search: Omit<InsertSavedSearch, 'userId'>): Promise<SavedSearch> {
    // @ts-ignore – drizzle-zod type inference mismatch; runtime values are correct
  ...
  async getAgentClients(agentEmail: string): Promise<(ClientAgentLink & { client: any })[]> {
    ...
    return results.map(r => ({ ...r.link, client: r.client ? safeUser(r.client as any) : null }));
  }
  ...
  async getBuyerProfiles(filters?: any): Promise<(BuyerProfile & { user: any })[]> {
    ...
    return results.map(r => ({ ...r.profile, user: r.user ? safeUser(r.user as any) : null }));
  }
  ...
  async getBuyerProfile(id: number): Promise<(BuyerProfile & { user: any }) | undefined> {
    ...
    if (results.length === 0) return undefined;
    return { ...results[0].profile, user: results[0].user ? safeUser(results[0].user as any) : null };
  }
  ...
  async matchBuyersForListing(criteria: {
    ...
    const scored = results.map(r => {
      ...
      return { ...r.profile, user: r.user ? safeUser(r.user as any) : null, ...s };
    });
  ...
  async getBuyerMatchesForProfile(profileId: number): Promise<(BuyerMatch & { property: Property | null; sender: any })[]> {
    ...
    return results.map(r => ({ ...r.match, property: r.property, sender: r.sender ? safeUser(r.sender as any) : null }));
  }
  ...
  async getSellerPitches(): Promise<(SellerPitch & { user: any })[]> {
    ...
    return results.map(r => ({ ...r.pitch, user: r.user ? safeUser(r.user as any) : null }));
  }
  ...
  async getSellerPitch(id: number): Promise<(SellerPitch & { user: any }) | undefined> {
    ...
    return { ...results[0].pitch, user: results[0].user ? safeUser(results[0].user as any) : null };
  }
  ...
  async getPropertyOffersForProperty(propertyId: number): Promise<(PropertyOffer & { property: Property; buyer: any })[]> {
    ...
    return rows.map(r => ({ ...r.offer, property: r.property, buyer: r.buyer ? safeUser(r.buyer as any) : null }));
  }
  ...
  async getSwipeNotificationsForUser(userId: string): Promise<(SwipeNotification & { property: Property; buyer: any })[]> {
    ...
    return rows.map(r => ({ ...r.notification, property: r.property, buyer: r.buyer ? safeUser(r.buyer as any) : null }));
  }
  ...
  async getPropertyReviews(propertyId: number): Promise<(PropertyReview & { user: any })[]> {
    ...
    return rows.map(r => ({ ...r.review, user: r.user ? safeUser(r.user as any) : null }));
  }
  ...
  async getConversationsForUser(userId: string, type?: string): Promise<(Conversation & { property: Property; buyer: any; agent: any; lastMessage: Message | null; unreadCount: number })[]> {
    ...
    for (const row of rows) {
      ...
      results.push({ ...conv, property: row.properties, buyer: buyer ? safeUser(buyer as any) : null, agent: agent ? safeUser(agent as any) : null, lastMessage, unreadCount });
    }
    return results;
  }
  ...
  async getAllConversations(filters?: { search?: string; status?: string; limit?: number; offset?: number }): Promise<{ conversations: (Conversation & { property: Property; buyer: any; agent: any; lastMessage: Message | null; messageCount: number })[]; total: number }> {
    ...
    for (const row of rows) {
      ...
      allResults.push({ ...conv, property: row.properties, buyer: buyer ? safeUser(buyer as any) : null, agent: agent ? safeUser(agent as any) : null, lastMessage, messageCount });
    }
  ...
  async getMessagesForConversation(conversationId: number, limit: number = 50, before?: number): Promise<(Message & { sender: any })[]> {
    ...
    for (const msg of rows) {
      ...
      results.push({ ...msg, sender: sender ? safeUser(sender as any) : null });
    }
  ...
  async claimPasswordResetToken(tokenHash: string): Promise<PasswordResetToken | undefined> {
    const now = new Date();
