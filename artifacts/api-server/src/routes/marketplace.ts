...
router.post("/api/buyer-matches", isAuthenticated, async (req, res) => {
  try {
    const { insertBuyerMatchSchema } = await import("@workspace/db");
    const parsed = (insertBuyerMatchSchema as z.ZodObject<any>).omit({ senderId: true, conversationId: true }).safeParse(req.body);
