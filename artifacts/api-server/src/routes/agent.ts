...
router.post("/api/agent/contacts/preview-vcard", isAuthenticated, async (req: any, res) => {
  ...
  try {
    ...
    const cards: any[] = VCard.parse(parsed.data.vcard);
...
router.patch("/api/agent/buyer-clients/:id", isAuthenticated, async (req, res) => {
  try {
    ...
    const { insertBuyerProfileSchema } = await import("@workspace/db");
    const parsed = (insertBuyerProfileSchema as z.ZodObject<any>).omit({ userId: true, agentId: true }).partial().safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ message: "Invalid data", errors: parsed.error.flatten() });
