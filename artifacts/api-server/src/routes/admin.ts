import { Router } from "express";
  ...
  import { safeUser, safeUsers } from "../utils/safeUser";
...
router.get("/api/admin/property-offers", isAuthenticated, isAdmin, async (_req, res) => {
  try {
    ...
    res.json(rows.map(r => ({ ...r.offer, property: r.property, buyer: r.buyer ? safeUser(r.buyer as any) : null })));
  } catch (err: any) {
    res.status(500).json({ message: err.message });
  }
});
...
router.get("/api/admin/buyer-pitches", isAuthenticated, isAdmin, async (_req, res) => {
  try {
    ...
    res.json(allMatches.map(r => ({
      ...
      sender: r.sender ? safeUser(r.sender as any) : null,
    })));
  } catch (err: any) {
    res.status(500).json({ message: err.message });
  }
...
router.get("/api/admin/buyer-referrals", isAuthenticated, isAdmin, async (_req, res) => {
  try {
    ...
    res.json(referrals.map(r => ({ ...r.profile, user: r.user ? safeUser(r.user as any) : null, type: "buyer" })));
  } catch (err: any) {
    res.status(500).json({ message: err.message });
  }
});
  ...
  try {
    ...
    const usersWithActivity = await Promise.all(
      filtered.map(async (u) => {
        ...
        return { ...safeUser(u as any), activity };
      })
    );
...
router.get("/api/admin/users/:id", isAuthenticated, isAdmin, async (req, res) => {
  try {
    ...
    res.json({ ...safeUser(user as any), activity });
  } catch (err: any) {
    res.status(500).json({ message: err.message });
  }
});
...
router.patch("/api/admin/users/:id", isAuthenticated, isAdmin, async (req, res) => {
  try {
    ...
    res.json(safeUser(updated as any));
  } catch (err: any) {
...
router.get("/api/admin/audit-events/stats", isAuthenticated, isAdmin, async (_req: any, res) => {
  try {
    const { auditEvents } = await import("@workspace/db");
