...
- **Hero Address Flow**: All three hero tabs (Buy, Sell, Estimate) use AddressAutocomplete. Sell and Estimate tabs pass typed address via `?address=` URL param to /sell and /home-report pages, which pre-fill their address fields. The Buy tab autocomplete shows both **location suggestions** (cities like "San Diego, CA" and counties like "San Diego County, CA" with listing counts) and individual property matches. Selecting a city navigates to `/search?city=X`, selecting a county to `/search?county=X`. Backend: `GET /api/locations/autocomplete?q=...` returns grouped location suggestions, `GET /api/properties?city=X` filters by exact city, `GET /api/properties?county=X` returns all listings in the county area.
