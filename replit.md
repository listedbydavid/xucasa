...
- **Property Details**: Enhanced with mortgage calculator (with PMI auto-calculation when down payment < 20%), neighborhood insights, share and print options. Includes **Agent MLS Panel** for verified agents showing confidential remarks, showing instructions, lockbox/access info, listing/co-listing agent details, buyer agent compensation, and MLS documents. **3D Virtual Tour**: Embeds MLS virtual tour (Matterport, Zillow 3D, etc.) in a responsive 16:9 iframe with collapse/expand toggle and "Open full tour" external link. Tours synced from RESO `VirtualTourURLUnbranded`/`VirtualTourURLBranded` fields and Media collection. Property cards show "3D Tour" badge for listings with tours. Schema field: `virtualTourUrl`.
...
- **Notification System**: In-app notifications with bell icon in navbar showing unread count badge, dropdown panel with notification list (mark read, archive, delete per item, mark all read), and full management tab in Dashboard with filters (all/unread/archived). Notification types: new_listing, price_drop, agent_match, open_house, system. Test notification seeder via `POST /api/notifications/test`. Routes: `GET /api/notifications`, `GET /api/notifications/unread-count`, `PATCH /api/notifications/mark-all-read`, `PATCH/DELETE /api/notifications/:id`. Component: `NotificationBell.tsx`. Dashboard section: `NotificationsSection` in `Dashboard.tsx`. DB table: `notifications`.
- **Email Notification System**: Gmail API-based email notifications via Replit Gmail connector (OAuth). Users can enable email and in-app alerts per notification type (new_listing, price_drop, open_house, agent_match, system) via functional toggle switches in the Dashboard Notification Preferences panel. Features: branded HTML email templates with xucasa styling, rate limiting (max 20 emails/user/day with automatic daily reset), deduplication by userId+type+propertyId (suppresses identical emails within 1 hour), "Send Test Email" button in preferences panel. Email sending is wired into notification creation flow — when a notification is created and the user has email enabled for that type, an email is sent in the background. Admin test email endpoint: `POST /api/admin/test-email`. Routes: `GET/PATCH /api/notification-preferences`, `GET /api/email-status`. DB table: `notification_preferences` (email + in-app channel preferences). Service: `server/emailService.ts` (uses googleapis package with Replit Gmail connector).
