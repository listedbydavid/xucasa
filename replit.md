...
- **Email Notification System**: SMTP-based email notifications via Nodemailer. Users can enable email alerts per notification type (new_listing, price_drop, open_house, agent_match, system) via functional toggle switches in the Dashboard Notification Preferences panel. Features: branded HTML email templates with xucasa styling, rate limiting (max 20 emails/user/day with automatic daily reset), deduplication (suppresses identical emails within 1 hour), SMTP configuration via env vars (SMTP_HOST, SMTP_USER, SMTP_PASS, optional SMTP_PORT/SMTP_FROM_NAME/SMTP_FROM_EMAIL). Email sending is wired into notification creation flow — when a notification is created and the user has email enabled for that type, an email is sent in the background. Admin test email endpoint: `POST /api/admin/test-email`. Routes: `GET/PATCH /api/notification-preferences`, `GET /api/email-status`. DB table: `notification_preferences`. Service: `server/emailService.ts`.
